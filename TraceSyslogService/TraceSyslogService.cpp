...
#include "shape__TraceSyslogService.hxx"

TRC_INIT_MODULE(shape::TraceSyslogService);

namespace shape {

  	class TraceSyslogService::Imp {
	private:
		std::mutex m_mtx;
	...
	TraceSyslogService::TraceSyslogService() {
		imp = shape_new Imp();
	}
	...
	bool TraceSyslogService::isValid(int level, int chan) const {
		return imp->isValid(level, chan);
	}

	void TraceSyslogService::writeMsg(int level, int channel, const char* moduleName, const char* sourceFile, int sourceLine, const char* funcName, const std::string & msg) {
		imp->writeMsg(level, channel, std::string(moduleName), std::string(sourceFile), sourceLine, funcName, msg);
	}

	void TraceSyslogService::activate(const Properties *props) {
		imp->modify(props);
	}

	void TraceSyslogService::deactivate() {
	}

	void TraceSyslogService::modify(const Properties *props) {
		imp->modify(props);
	}
